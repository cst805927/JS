...
- 已知 for-of 循环会在相关对象上使用 Symbol.iterator 属性，
  ...
  - 通过修改在自定义对象上重新定义 Symbol.iterator 的值
...
- 如果这个布尔值是true，意味着什么？
  ...
  - 意味着对象应该用 Array.prototype.concat()
...
- 如何修改 Array.prototype.concat()方法的行为？
  ...
  - 覆盖 Symbol.isConcatSpreadable 的值

    可以修改该行为

- 数组对象默认情况下会发生什么？
  - 会被打平到已有的数组
- Symbol.isConcatSpreadable符号为 false 或假值会导致什么？
  - 导致整个对象被追加到数组末尾